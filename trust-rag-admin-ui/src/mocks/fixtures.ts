...
import type { DocumentImportTask, KnowledgeItem, KnowledgeReferenceItem, PromotionTask } from '@/api/knowledge'
...
export const mockReviewCandidates: KnowledgeItem[] = [
  ...
  ...Array.from({ length: 12 }, (_, index): KnowledgeItem => ({
    id: 9101 + index,
    title: `低可信候选知识 ${index + 1}`,
    content: `这是一条来自用户纠错或候选抽取的低可信知识，需要 Promotion 晋升任务评估后才能进入中可信审核池。序号：${index + 1}`,
    trustLevel: 'LOW',
    status: index % 3 === 0 ? 'LOW_PENDING' : 'LOW_ENABLED',
    scopeType: index % 2 === 0 ? 'GLOBAL_CANDIDATE' : 'PROJECT',
    sourceType: index % 2 === 0 ? 'user_correction' : 'manual',
    sourceRef: `mock://low/${index + 1}`,
  ...
  })),
  {
    ...
    sourceRef: 'mock://promotion/rejected',
  ...
  },
  {
    id: 9202,
    title: '晋升异常：和高可信知识冲突',
    content: '这条候选知识与已有高可信知识存在事实冲突。',
    trustLevel: 'LOW',
    status: 'CONFLICT',
    scopeType: 'PROJECT',
    sourceType: 'manual',
    sourceRef: 'mock://promotion/conflict',
    rejectReason: 'Conflict risk exceeds promotion limit',
    confidence: 0.61,
    privacyScore: 0,
    projectId: 'trust-rag',
    tenantId: 'demo',
  ...
  },
  {
    id: 9203,
    title: '晋升异常：等待合并',
    content: '这条候选知识和已有知识高度重复，需要合并到正确版本。',
    trustLevel: 'LOW',
    status: 'MERGE_PENDING',
    scopeType: 'PROJECT',
    sourceType: 'user_correction',
    sourceRef: 'mock://promotion/merge',
    rejectReason: 'LLM pre-review requires a merge',
    confidence: 0.68,
    privacyScore: 0,
    projectId: 'trust-rag',
    tenantId: 'demo',
  ...
  },
  {
    id: 9204,
    title: '晋升异常：索引写入失败',
    content: '这条候选已通过晋升判断，但写入索引失败，等待补偿重试。',
    trustLevel: 'MEDIUM',
    status: 'INDEX_FAILED',
    scopeType: 'GLOBAL',
    sourceType: 'internal_doc',
    sourceRef: 'mock://promotion/index-failed',
    rejectReason: 'LOW_TO_MEDIUM_INDEX_FAILED',
    confidence: 0.82,
    privacyScore: 0,
    createdAt: now,
    updatedAt: now,
...
export const mockPromotionTasks: PromotionTask[] = [
  {
    id: 501,
    knowledgeId: 9201,
    status: 'SUCCESS',
  ...
  },
  {
    ...
    taskType: 'LOW_TO_MEDIUM',
    retryCount: 1,
    errorMessage: 'LOW_TO_MEDIUM_INDEX_FAILED',
    startedAt: now,
    finishedAt: now,
    createdAt: now,
  },
